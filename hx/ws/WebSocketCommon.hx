...
class WebSocketCommon {
                ...
                switch (_opcode) {
                    ...
                    case OpCode.Close:
                        if(_payload.length == 0) {
                            handleClose(1005);
                        } else if(_payload.length == 1) {
                            if(onerror != null) {
                                onerror('Invalid payload length 1');
                            }

                            handleClose(1002);
                        } else {
                            var code:Int = _payload.readUnsignedShort();
                            var msg:String = Utf8Encoder.decode(_payload.readAllAvailableBytes());

                            handleClose(code, msg);
                        }
                }
    ...
    private function handleClose(code:Int = 0, ?reason:String) {
        if(state != State.Closed) {
            try {
                Log.debug("Closed", id);
                state = State.Closed;
            ...
            if (onclose != null) {
                if(code > 999 && reason != null) {
                    onclose(code, reason);
                } else {
                    onclose(0, 'Connection Failed');
                }
            }
        }
    }
